...
async function monitor(region, juststarted, oldProducts, lastcache) {
    ...
    try {
        ...
        let method = 'GET';
        let req = `https://api.nike.com/product_feed/threads/v2?anchor=${region.CHANNELID}&count=100&filter=exclusiveAccess(true,false)&filter=channelId(d9a5bc42-4b9c-4976-858a-f159cf99c647)&filter=marketplace(${region.MARKETPLACE})&filter=language(${region.LANGUAGE})&filter=taxonomyIds(0d17e0df-9083-4e44-b263-71c2565d4783,16633190-45e5-4830-a068-232ac7aea82c)&filter=productInfo.merchProduct.channels(NikeApp)&fields=active,id,lastFetchTime,productInfo,${v4()}`//request url can add &searchTerms=dunk%20low
        ...
        for (let product of body.objects) {
            ...
            let catalogid = productInfo.merchProduct.catalogId
            ...
            let links = `[ATF](http://api.YOUmonitors.com/nike?pid=${pid}) | [Nikeweb](${url}) | [Stockx](https://stockx.com/search?s=${sku}) | [Goat](https://goat.com/search?query=${sku}) | [Token](https://www.YOUmonitors.com/token-save)`
            //https://www.nike.com/us/en/launch-checkout?productId=acafd216-6a3e-590e-b105-0359ad159ab4&channel=Nike.com&threadId=c9c42e3f-ffc6-40ce-8f47-126c9b1422ab&size=11.5
            let sizes = '';
