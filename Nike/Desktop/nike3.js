...
async function monitor(region, juststarted, oldProducts) {
    try {
        ...
        let method = 'GET';
        let req = `https://api.nike.com/product_feed/threads/v2?anchor=${region.CHANNELID}&count=100&filter=exclusiveAccess(true,false)&filter=channelId(82a74ac1-c527-4470-b7b0-fb5f3ef3c2e2)&filter=marketplace(${region.MARKETPLACE})&filter=language(${region.LANGUAGE})&filter=taxonomyIds(0d17e0df-9083-4e44-b263-71c2565d4783,16633190-45e5-4830-a068-232ac7aea82c)&filter=productInfo.merchProduct.channels(NikeApp)&fields=active,id,lastFetchTime,productInfo,${v4()},${v4()}`//request url can add &searchTerms=dunk%20low
