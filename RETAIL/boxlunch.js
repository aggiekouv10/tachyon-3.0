...
const site = 'BOXLUNCH'; //site name
const catagory = 'RETAIL'
const version = `Boxlunch v1.0` //Site version
...
async function monitor(sku) {
    try {
        ...
        let req = `https://www.boxlunch.com/on/demandware.store/Sites-boxlunch-Site/default/Product-Variation?pid=${sku}&format=ajax&quantity=1000000`//request url
        ...
        console.log(stock)
            if (stock > 0 || stock == 'Presale') {
                let url = `https://www.boxlunch.com/product/YOU/${sku}.html`
                if (status !== "STOCK") {
                    let qt = 'NA'
                    let links = `[ATC](https://www.boxlunch.com/product/YOU/${sku}.html)`
